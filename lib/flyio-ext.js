...
class FlyIOExt {
  ...
  get(name) {
    ...
    let fullUrl = childOpt.url;
    if(rootOpt.config && rootOpt.config.baseURL) {
      fullUrl = rootOpt.config.baseURL + '/' +fullUrl;
    }
    //修复替换bug
    fullUrl = fullUrl.replace(/:?\/\//g, function(v){
      return v === '//' ? '/' : v;
    })
    return {... childOpt, url: fullUrl, config: rootOpt.config || {}}
  }
